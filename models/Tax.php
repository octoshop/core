...
class Tax extends Model
{
    ...
    public static function calculateTaxes($items, $locationInfo)
    {
        ...
        foreach ($items as $itemIndex => $item) {
            if (!($taxClass = static::findById($item->tax_class_id))) {
                continue;
            }
    ...
    public function getTaxRates($amount, $locationInfo)
    {
        ...
        foreach ($addedTaxes as $addedTax) {
            $result[] = (object) [
                'name' => $addedTax->name,
                'tax_rate' => $taxRate = $addedTax->rate / 100,
                'rate' => $addedResult += $taxRate = round($amount * $taxRate, 2),
                'total' => $rate,
                'added_tax' => true,
                'compound_tax' => false,
            ];
        ...
        foreach ($compoundTaxes as $compoundTax) {
            $result[] = (object) [
                'name' => $compoundTax->name,
                'tax_rate' => $taxRate = $compoundTax->rate / 100,
                'rate' => $rate = round($addedResult * $taxRate, 2),
                'total' => $rate,
                'compound_tax' => true,
                'added_tax' => false,
            ];
